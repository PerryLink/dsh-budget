...
| 方面 | 状态 |
|---|---|
| Harness | DeepSeek Harness `dsh-v0.1.7-alpha.1`（GitHub tag，2026-09-18 已适配；peer 范围 `>=0.1.2-rc.1 <0.2.0 || >=0.1.5-alpha.1 <0.2.0 || >=0.1.6-0 <0.2.0`）：alpha.2 目录价已内置进价目表，未定价模型以「未定价」呈现而非虚构估算；审计门继续抑制 `budget/alert`/`budget/block` 落盘（fail-closed 会话事件词表）。已于 2026-09-18 通过双尺子 typecheck 链与完整本地门禁核验；浏览器面板项为 人工·未测即未完成（维护者人工清单）。 |
