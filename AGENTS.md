...
- `tests/` — vitest; REAL `Context` + REAL `SessionStore`/`Session` from the `0.1.7-rc.2` peers; scripted `commands`/`llm` stand-ins (the plugin reads them as optional services); the estimate layer carries upstream fixture numbers as regressions.
...
`pnpm run typecheck && pnpm run typecheck:ci && pnpm test && pnpm run build && pnpm run verify:self-contained && pnpm run verify:artifacts && pnpm pack`. The plain `typecheck` resolves the local harness checkout's type faces through tsconfig `paths`; `typecheck:ci` resolves the npm-published `0.1.7-rc.2` faces (no paths) and is what CI runs — keep both green.
