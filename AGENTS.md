...
- `tests/` — vitest; REAL `Context` + REAL `SessionStore`/`Session` from the `0.1.2-alpha.5` peers; scripted `commands`/`llm` stand-ins (the plugin reads them as optional services); the estimate layer carries upstream fixture numbers as regressions.
...
- **Audit event gate**: `budget/alert` + `budget/block` appends are gated by the installed `@deepseek-ai/dsh-session` line version (`auditAppendsAllowed` in `src/events.ts`): pre-0.1.2-alpha.1 lines keep writing; 0.1.2-alpha.1+ suppress and log the degradation reason. 0.1.2-alpha.5 keeps the suppression: its envelope field is retained for stored-log read compatibility only and `Session.append` still cannot stamp the marker.
...
`pnpm run typecheck && pnpm run typecheck:ci && pnpm test && pnpm run build && pnpm run verify:self-contained && pnpm run verify:artifacts && pnpm pack`. The plain `typecheck` resolves the local harness checkout's type faces through tsconfig `paths`; `typecheck:ci` resolves the npm-published `0.1.2-alpha.5` faces (no paths) and is what CI runs — keep both green.
